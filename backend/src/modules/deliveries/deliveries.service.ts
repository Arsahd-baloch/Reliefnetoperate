...
export class DeliveriesService {
   ...
   */
  async submitDelivery(input: SubmitDeliveryInput & { quantity_delivered: number }, volunteerId: number) {
    ...
    try {
      ...
      const taskResult = await client.query(
        `SELECT id, claimed_by, status, items_needed FROM tasks WHERE id = $1 FOR UPDATE`,
        [input.task_id]
      );
      ...
      const deliveryResult = await client.query(
        `INSERT INTO deliveries (task_id, volunteer_id, storage_keys, gps_latitude, gps_longitude, notes, quantity_delivered, status)
    ...
    try {
      ...
      if (verifierRole === 'COORDINATOR') {
        const jurisdictionCheck = await client.query(
          `SELECT 1 FROM tasks WHERE id = $1 AND (coordinator_id = $2 OR coordinator_id IS NULL)`,
