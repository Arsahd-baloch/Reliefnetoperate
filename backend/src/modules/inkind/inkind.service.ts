...
export class InKindService {
  ...
  async createRequest(donationId: number, beneficiaryId: number, input: CreateInKindRequestInput) {
    ...
    try {
      ...
      await pool.query(
        ...
        [roomResult.rows[0].id, request.id]
      );

      // Notify donor about new request
      notificationService.notifyInKindRequest(
        donation.rows[0].donor_id,
        donationId,
        donation.rows[0].title,
        'A beneficiary' // Could fetch name if needed
      );
  ...
  async rejectRequest(requestId: number, donorId: number) {
    ...
    try {
      ...
      const reqResult = await client.query(
        `SELECT r.id, r.donation_id, r.beneficiary_id, r.status, d.donor_id, d.title AS donation_title
         FROM inkind_requests r
         JOIN inkind_donations d ON d.id = r.donation_id
         WHERE r.id = $1
         FOR UPDATE`,
  ...
  async completeDonation(donationId: number, beneficiaryId: number) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');

      const donationResult = await client.query(
        `SELECT d.id, d.title, d.donor_id, d.status, r.beneficiary_id
         FROM inkind_donations d
      ...
      if (donationResult.rows.length === 0) throw createError('Donation or accepted request not found', 404);
      const donation = donationResult.rows[0];

      if (donation.beneficiary_id !== beneficiaryId) throw createError('Only the assigned beneficiary can confirm receipt', 403);
      if (donation.status !== 'ACCEPTED') throw createError(`Donation cannot be completed from ${donation.status} status`, 409);

      await client.query(
        `UPDATE inkind_donations SET status = 'COMPLETED', updated_at = NOW() WHERE id = $1`,
        [donationId]
      );
