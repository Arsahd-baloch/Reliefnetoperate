...
export class TasksService {
  ...
  async createTask(input: CreateTaskInput, createdBy: number): Promise<TaskRow> {
    ...
    ).catch(err => console.error('[EVENT] Failed to record task creation:', err));
  ...
  async updateTask(id: number, input: UpdateTaskInput, userId: number, role: string) {
    ...
    if (role !== 'ADMIN') {
      if (role === 'COORDINATOR') {
        ...
        const jurisdictionCheck = await pool.query(
   ...
   */
  async assignTask(taskId: number, volunteerId: number, adminId: number) {
    ...
    try {
      ...
      const taskResult = await client.query(
        `SELECT id, status, title FROM tasks WHERE id = $1 FOR UPDATE`,
        [taskId]
      );
      if (taskResult.rows.length === 0) throw createError('Task not found', 404);
      const task = taskResult.rows[0];
      if (task.status !== 'OPEN') throw createError('Task is not OPEN', 400);

      // 2. Validate volunteer
      const volCheck = await client.query(
        'SELECT status FROM volunteer_profiles WHERE user_id = $1',
        [volunteerId]
      );
      ...
      // 3. Assign
      await client.query(
        `UPDATE tasks SET status = 'ASSIGNED', claimed_by = $1, claimed_at = NOW() WHERE id = $2`,
        [volunteerId, taskId]
      );
      ...
      await client.query(
        ...
        [taskId, adminId, JSON.stringify({ assigned_to: volunteerId })]
      );
