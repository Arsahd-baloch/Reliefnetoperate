...
export class DonationsService {
  async createStripeSession(input: CreateDonationInput, donorId: number) {
    const campaignResult = await pool.query(
      `SELECT id, title, status FROM campaigns WHERE id = $1 AND deleted_at IS NULL`,
      [input.campaign_id]
    );
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      try {
        ...
        const existing = await client.query(
          'SELECT id FROM donations WHERE gateway_ref = $1',
          [session.id]
        );
        ...
        if (existing.rows.length === 0) {
          const result = await client.query(
            `INSERT INTO donations
             ...
             RETURNING id`,
            [parseInt(donor_id), parseInt(campaign_id), parseFloat(amount_pkr), session.id]
          );
          ...
          await client.query(
            ...
            [parseFloat(amount_pkr), parseInt(donor_id), donationId]
          );
        }
  ...
  async approveDonation(donationId: number, adminId: number, ip?: string, role?: string) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');

      const donationResult = await client.query(
        `SELECT d.id, d.donor_id, d.campaign_id, d.amount_pkr, d.status, c.ngo_id
         FROM donations d
      ...
      if (role === 'NGO') {
        const ngoCheck = await client.query(
          'SELECT id FROM ngo_profiles WHERE user_id = $1 AND id = $2',
          [adminId, donation.ngo_id]
        );
  ...
  async rejectDonation(donationId: number, adminId: number, ip?: string, role?: string) {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
  ...
  async getDonationsByNgo(ngoUserId: number, status?: string) {
    const conditions = ['c.ngo_id = (SELECT id FROM ngo_profiles WHERE user_id = $1)'];
    const values: unknown[] = [ngoUserId];
