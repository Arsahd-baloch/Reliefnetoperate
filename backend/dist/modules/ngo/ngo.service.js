...
export class NgoService {
    ...
    async getNgoIdByUserId(userId) {
        const result = await pool.query('SELECT id FROM ngo_profiles WHERE user_id = $1 AND deleted_at IS NULL', [userId]);
     ...
     * Public profile data.
     */
    async getPublicProfile(ngoId) {
        const [profile, campaigns] = await Promise.all([
            pool.query('SELECT org_name, registration_number, status, verified_at, created_at, bank_name, account_title, account_number FROM ngo_profiles WHERE id = $1 AND deleted_at IS NULL', [ngoId]),
            pool.query(`
