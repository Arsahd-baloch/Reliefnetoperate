...
export class AdminService {
    applyPagination(query, options, values) {
    ...
    async getAllDonations(status, options = {}) {
        let baseQuery = `
    ...
    async getAllWithdrawals(status, options = {}) {
        let baseQuery = `
    ...
    async getAuditLogs(options = {}, filters = {}) {
        let baseQuery = `
    ...
    `, [id]);
        if (ngoResult.rows.length === 0)
            throw createError('NGO not found', 404);
    ...
    async verifyNgo(id, adminId, ip) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            const result = await client.query(`UPDATE ngo_profiles SET status = 'VERIFIED', verified_at = NOW() WHERE id = $1 RETURNING *`, [id]);
         ...
         VALUES ($1, 'VERIFY_NGO', 'ngo_profiles', $2, $3, $4)`, [adminId, id, JSON.stringify({ org_name: ngo.org_name }), ip]);
    ...
    async rejectNgo(id, adminId, reason, ip) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            const result = await client.query(`UPDATE ngo_profiles SET status = 'REJECTED' WHERE id = $1 RETURNING *`, [id]);
            if (result.rows.length === 0)
                throw createError('NGO profile not found', 404);
            const ngo = result.rows[0];
            await client.query(`INSERT INTO audit_logs (admin_id, action_type, target_entity, target_id, metadata, ip_address)
         VALUES ($1, 'REJECT_NGO', 'ngo_profiles', $2, $3, $4)`, [adminId, id, JSON.stringify({ org_name: ngo.org_name, reason }), ip]);
            await client.query('COMMIT');
            return ngo;
        }
        catch (err) {
    ...
    // --- Financial Safety ---
    async flagDonation(id, adminId, reason, ip) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            const result = await client.query(`UPDATE donations SET metadata = COALESCE(metadata, '{}'::jsonb) || $2::jsonb WHERE id = $1 RETURNING *`, [id, JSON.stringify({ disputed: true, dispute_reason: reason, flagged_at: new Date().toISOString() })]);
            if (result.rows.length === 0)
                throw createError('Donation not found', 404);
            await client.query(`INSERT INTO audit_logs (admin_id, action_type, target_entity, target_id, metadata, ip_address)
         VALUES ($1, 'FLAG_DONATION', 'donations', $2, $3, $4)`, [adminId, id, JSON.stringify({ reason }), ip]);
            await client.query('COMMIT');
            return result.rows[0];
        }
        catch (err) {
    ...
    async flagWithdrawal(id, adminId, reason, ip) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            const result = await client.query(`UPDATE withdrawals SET metadata = COALESCE(metadata, '{}'::jsonb) || $2::jsonb WHERE id = $1 RETURNING *`, [id, JSON.stringify({ flagged: true, flag_reason: reason, flagged_at: new Date().toISOString() })]);
            if (result.rows.length === 0)
                throw createError('Withdrawal not found', 404);
            await client.query(`INSERT INTO audit_logs (admin_id, action_type, target_entity, target_id, metadata, ip_address)
         VALUES ($1, 'FLAG_WITHDRAWAL', 'withdrawals', $2, $3, $4)`, [adminId, id, JSON.stringify({ reason }), ip]);
            await client.query('COMMIT');
            return result.rows[0];
        }
        catch (err) {
